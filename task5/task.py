...
def make_experts_answer(controversy, trans):
    ...
    for value in trans.values():
        if value not in visited:
            ...
            result.append(list(new_cluster) if len(new_cluster) > 1 else new_cluster.pop())

    return result

def task():
    a_data = read_json('ranj_A.json')
    b_data = read_json('ranj_B.json')
    ...
    return make_experts_answer(controversy, trans)
